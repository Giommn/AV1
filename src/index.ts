...
function stringParaNivelPermissao(valor: string): NivelPermissao {
    if (!valor) {
        return NivelPermissao.OPERADOR;
    }
    
    const chave = Object.keys(NivelPermissao).find(key => NivelPermissao[key as keyof typeof NivelPermissao].toLowerCase() === valor.toLowerCase());
    return NivelPermissao[chave as keyof typeof NivelPermissao] || NivelPermissao.OPERADOR;
}

async function escolherItem<T>(lista: T[], mensagem: string, display: (item: T) => string): Promise<T | null> {
    if (lista.length === 0) {
        return null;
    }
...
async function cadastrarNovaAeronave() {
    ...
    const todasAeronaves = carregarDados<Aeronave>(arquivo_aeronaves);
...
async function cadastrarNovaEtapa() {
    ...
    const aeronaveAlvo = todasAeronaves.find(a => a.codigo === aeronave.codigo);
    if (aeronaveAlvo) {
        if(!aeronaveAlvo.etapas) aeronaveAlvo.etapas = [];
...
async function realizarTesteEmAeronave() {
    ...
    const tipoInput = await perguntar('Tipo de teste (Eletrico, Hidraulico, Aerodinamico): ') as TipoTeste;
    const resultadoInput = await perguntar('Resultado (Aprovado, Reprovado): ') as ResultadoTeste;

    const novoTeste = new Teste(tipoInput, resultadoInput);

    const aeronaveAlvo = todasAeronaves.find(a => a.codigo === aeronave.codigo);
    if(aeronaveAlvo) {
        if(!aeronaveAlvo.testes) aeronaveAlvo.testes = [];
...
async function gerarRelatorioFinal() {
    ...
    const aeronaveCompleta = carregarDados<Aeronave>(arquivo_aeronaves).find(a => a.codigo === aeronave.codigo);
    if(aeronaveCompleta) {
...
async function cadastrarNovoFuncionario() {
    ...
    const todosOsFuncionarios = carregarDados<Funcionario>(arquivo_funcionarios);
...
function listarFuncionarios(solicitante: Funcionario) {
...
async function realizarLogin(): Promise<Funcionario | null> {
    ...
    const funcionarioEncontrado = todosOsFuncionarios.find(f => {
        const permissaoEnum = stringParaNivelPermissao(f.nivelPermissao as unknown as string);
    ...
    if (funcionarioEncontrado) {
        ...
        const permissaoEnum = stringParaNivelPermissao(funcionarioEncontrado.nivelPermissao as unknown as string);
...
async function cadastrarPrimeiroAdministrador(): Promise<Funcionario | null> {
...
async function menuPrincipal(funcionarioLogado: Funcionario) {
    let loop = true;
    while (loop) {
        ...
        switch (opcao) {
            case '1': await cadastrarNovaAeronave(); break;
            case '2': listarAeronaves(); break;
            case '3': await associarPecaAeronave(); break;
            case '4': await cadastrarNovaEtapa(); break;
            case '5': await realizarTesteEmAeronave(); break;
            case '6': await gerarRelatorioFinal(); break;
            case '7': await cadastrarNovoFuncionario(); break;
            case '8': listarFuncionarios(funcionarioLogado); break;
            case '0': loop = false; console.log('\nSaindo do sistema...'); break;
...
async function iniciarSistema() {
    const todosFuncionarios = carregarDados<Funcionario>(arquivo_funcionarios);
